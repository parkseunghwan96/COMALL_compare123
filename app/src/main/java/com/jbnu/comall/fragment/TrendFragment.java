...
public class TrendFragment extends BaseFragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        TextListAdapter adapter = new TextListAdapter(categories, (v, pos) -> {
            ...
            DBHelper.getInstance().getTrend(category, trendList -> {
                TrendListFragment trendlistFragment = new TrendListFragment(trendList);
                FragmentTransaction transaction = getActivity().getSupportFragmentManager().beginTransaction();
                replaceWithStack(transaction, R.id.fragment_container, trendlistFragment);
            });
        });
